...
A.8 Digital Twin Correlation

Simulate each run in the 2-D heat/evaporation model:

input flux (q = 1.2–1.8 kW m⁻²), m₀ = 0.1, t = 0–900 s;

record predicted surface T(t), oxidation dose D = ∫ f(T) dt;

confirm experimental tone matches theoretical dose curve within ± 15 %.
This validates coupling between model and bench data.


A.9 Failure Modes & Corrections

Symptom	Cause	Correction

Deep brown penetration	Excess T > 180 °C or dry cloth	Increase distance / moisture.
Patchy tone	Uneven wetting or flux	Re-mist, rotate cloth.
Soot transfer	Overheating lamp/torch	Clean source lens, reduce flux.
No image	Too cool (<120 °C) or too wet (>20 %)	Adjust parameters.


A.10 Interpretation

If the resulting image passes superficiality, pigment-free, and mask tests, it provides empirical support for Leonardo’s capacity to generate the Shroud-like image via a controlled damp-heat photothermal process.
The reproducibility across three replicates establishes this as a viable proof-of-concept model—bridging STURP’s micro-forensics with Renaissance-era physics.
